...
import org.dexpi.pid.imaging.ImageMapObject;
...
/**
 * The svg imageFactory creates the svg and fills it with geometric shapes. The
...
public class SvgImageFactory implements GraphicFactory {
    ...
    public SvgImageFactory(Writer writer) {
        this.writer = writer;
    }
     ...
     * geometric shapes.
     *
     * @param resolutionX     the desired resolution in x direction (in pixels)
     * @param origin          the point zero specified in the "Proteus" document
     * @param size            the size specified in the "Proteus" document
    ...
    public void init(int resolutionX, double[] origin, double[] size,
        ...
        Element root = this.g.getRoot();
        root.setAttributeNS(null, "viewBox", "0 0 800 600");

        this.g.setSVGCanvasSize(new Dimension(6000, 7000));
     ...
     *
     * @param c            the color
     * @param lineWeight   the lineweight
     * @param xCoordinates the x-coordinates of the line
     * @param yCoordinates the y-coordinates of the line
     */
    @Override
     ...
     * @param c            the color
     * @param lineWeight   the lineweight
     * @param xCoordinates the x-coordinates of the shape
     * @param yCoordinates the y-coordinates of the shape
     * @param filled       if filled is set to true the shape will be filled with color
     */
    @Override
     ...
     * @param primAxis   the height/primary axis of the ellipse
     * @param secAxis    the width/secondary axis of the ellipse
     * @param startAngle the startAngle of the arc
     * @param endAngle   the endAngle of the arc
     * @param filled     if filled is set to true the ellipse will be filled with color
     */
    @Override
     ...
     * @param startAngle the startAngle of the arc
     * @param endAngle   the endAngle of the arc
     * @param filled     if filled is set to true the circle will be filled with color
     */
    @Override
    ...
    public void addText(Color c, double[] position, double[] extent,
        ...
        // rotate
        this.g.rotate(Math.toRadians(-textAngle), rotPt[0], rotPt[1]);
     ...
     * shapeCatalogue
     *
     * @param reference the reference, rotates the image. The two numbers represent
     *                  the cosinus and sinus of the desired angle.
     * @param scale     the scale
     * @param position  the position a shapeCatalogue-element should be drawn to
     */
    @Override
     ...
     *
     * @param pidElement the pidElement
     */
    @Override
    public void addImageMapElement(PidElement pidElement) {
        ...
        if (pidElement.getComponentClass() != null) {
            if (pidElement.getExtent() != null && pidElement.getType() == null
                    && !pidElement.getComponentClass().contains("Line")) {

                double[] extent = pidElement.getExtent();
                double[] position = pidElement.getPosition();

                String id = pidElement.getID();
                String componentName = pidElement.getComponentName();
                int startX, startY, rangeX, rangeY;

                startX = (int) ((extent[0] - this.x0) * this.x);
                rangeX = (int) ((extent[1] - extent[0]) * this.x);
                startY = (int) ((this.y0 - extent[3]) * this.x);
                rangeY = (int) ((extent[3] - extent[2]) * this.x);
